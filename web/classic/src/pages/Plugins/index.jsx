...
const Plugins = () => {
  ...
  const displayContent = async () => {
    setContent(localStorage.getItem('plugin_page_content') || '');
    const res = await API.get('/api/plugin_page_content');
    const { success, message, data } = res.data;
    if (success && data) {
      const isUrl = detectIsUrl(data);
      const isHtml = !isUrl && detectIsHtml(data);
      const pluginContent = isUrl || isHtml ? data : marked.parse(data);
      setContent(pluginContent);
      localStorage.setItem('plugin_page_content', pluginContent);
    } else if (!success && message) {
      showError(message);
      setContent('');
    } else {
      setContent('');
      localStorage.removeItem('plugin_page_content');
    }
    setLoaded(true);
  };
