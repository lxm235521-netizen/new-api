...
const KEY = 'task_per_call_billing_setting.model_names';
...
function textToApi(text) {
  return JSON.stringify(parseModelNames(text));
}

export default function SettingsTaskPerCallBilling(props) {
  ...
  return (
    <>
      <Spin spinning={loading}>
        ...
        >
          <Form.Section text={t('任务按次计费设置')}>
            <Row>
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.TextArea
                  label={t('模型列表')}
                  extraText={t(
                    '列表中的任务模型将按请求次数计费，不再叠加时长、尺寸等倍率。修改后对新任务立即生效。',
                  )}
